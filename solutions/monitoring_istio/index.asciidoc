...
== Requirements
* adapt the walkthrough of deploying the sample application to deploy your own application
* split your cluster in 3 seperate namespaces (shown below)


== Goals
Our goal is to eventually have a cluster with 3 namespaces: Application, Istio, Monitoring. On the application namespace we want to run our standard microservices. The Istio namespace is simply for the standard istio namespace. The monitoring namespace is needed to monitor and analyze our cluster. It should be possible that the namespaces communicate with each other. In detail we want to intercept the metrics that istio creates from the microservices and then read and analyze them via our monitoring namespace. 
